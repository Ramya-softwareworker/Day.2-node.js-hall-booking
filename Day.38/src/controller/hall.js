...
const bookingRoom = (req, res) => {
  try {
    ...
    if (idExists === undefined) {
      return res
        .status(400)
        .send({ message: "Room does not exist.", RoomsList: rooms });
    }
    ...
    res
      .status(400)
      .send({ message: "Error booking room", error: error, data: bookings });
  }
};
...
const getBookingCountByCustomer = (req, res) => {
  ...
  const customerBookings = customer.bookings.map((booking) => {
    const { customer, roomId, startTime, endTime, status, bookingDate } =
      booking;
